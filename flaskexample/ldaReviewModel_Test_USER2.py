...
def ldaReviewModel_Test_USER2(user_review_texts):
    ...
    if num_of_reviews:
        for topic in range(Num_Selected_Topics):
            combined_result[topic] = float(result[topic])/num_of_reviews
    average_score = sum(combined_result)/Num_Selected_Topics
    ...
    return improve_topic_keywords, combined_result, average_score
